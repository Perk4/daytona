...
export class ApiKeyService {
  ...
  async handleOrganizationDeletedEvent(payload: OrganizationDeletedEvent): Promise<void> {
    ...
    // Invalidate caches
    try {
      const cacheKeys = apiKeys.map((apiKey) => getApiKeyValidationCacheKey(apiKey.keyHash))
      const userCacheKeys = apiKeys.map((apiKey) => getApiKeyUserCacheKey(apiKey.userId))
      if (cacheKeys.length > 0 || userCacheKeys.length > 0) {
        await this.redis.del(...cacheKeys, ...userCacheKeys)
      }
    } catch (error) {
